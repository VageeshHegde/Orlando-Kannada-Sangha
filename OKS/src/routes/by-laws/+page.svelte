<script>
  ...
  onMount(async () => {
    try {
      // Get the by-laws PDF from Supabase Storage
      const { data: signedUrlData, error: signedUrlError } = await supabase.storage
        .from('OKS')
        .createSignedUrl('documents/OKS_Bylaws.pdf', 3600); // 1 hour expiration
      ...
      console.error('❌ Error loading by-laws PDF:', err);
      error = err.message;
      loading = false;
    }
  });
        ...
        <div class="pdf-viewer-wrapper">
          {#if loading}
            <div class="pdf-loading">
              <div class="text-center">
                ...
                <p class="mt-3">Loading By-Laws PDF...</p>
              </div>
            </div>
          {/if}
          ...
          {#if error}
            <div class="alert alert-danger">
              ...
              <br><small>Please make sure the OKS_Bylaws.pdf file exists in the 'documents' folder of your Supabase Storage.</small>
            </div>
          {/if}
